...
export const MacbookScroll = ({
  ...
  return (
    <div ref={ref} className="flex min-h-[200vh] shrink-0 scale-[0.35] transform flex-col items-center justify-start py-0 [perspective:800px] sm:scale-50 md:scale-100 md:py-80">
      <motion.h2 style={{ translateY: textTransform, opacity: textOpacity }}
        className="mb-20 text-center text-3xl font-bold text-white">
        {title || <span>Lumina Medical AI Suite</span>}
      </motion.h2>
      ...
      <div className="relative -z-10 h-[22rem] w-[32rem] overflow-hidden rounded-2xl bg-[#272729]">
        ...
        {showGradient && <div className="absolute inset-x-0 bottom-0 z-50 h-40 w-full bg-gradient-to-t from-black via-black to-transparent" />}
...
export const Lid = ({ scaleX, scaleY, rotate, translate, src }: {
  ...
  rotate: MotionValue<number>; translate: MotionValue<number>; src?: string;
}) => (
  <div className="relative [perspective:800px]">
    <div style={{ transform: "perspective(800px) rotateX(-25deg) translateZ(0px)", transformOrigin: "bottom", transformStyle: "preserve-3d" }}
      className="relative h-[12rem] w-[32rem] rounded-2xl bg-[#010101] p-2">
      <div style={{ boxShadow: "0px 2px 0px 2px #171717 inset" }}
        className="absolute inset-0 flex items-center justify-center rounded-lg bg-[#010101]">
        <span className="text-white"><LuminaLogo /></span>
      </div>
    </div>
    <motion.div style={{ scaleX, scaleY, rotateX: rotate, translateY: translate, transformStyle: "preserve-3d", transformOrigin: "top" }}
      className="absolute inset-0 h-96 w-[32rem] rounded-2xl bg-[#010101] p-2">
      <div className="absolute inset-0 rounded-lg bg-[#272729]" />
      {src && <img src={src} alt="screen" className="absolute inset-0 h-full w-full rounded-lg object-cover object-left-top" />}
    </motion.div>
  </div>
);
...
const LuminaLogo = () => (
  <svg width="48" height="48" viewBox="0 0 24 24" fill="none" className="text-teal-400">
    <circle cx="12" cy="12" r="4" stroke="#8daba8" strokeWidth="2" />
    <path d="M12 2v4M12 18v4M2 12h4M18 12h4" stroke="#8daba8" strokeWidth="1.5" strokeLinecap="round" />
    <circle cx="12" cy="12" r="8" stroke="#5c7f7c" strokeWidth="0.5" strokeDasharray="2 2" />
  </svg>
);
...
export const Keypad = () => (
  <div className="mx-1 h-full [transform:translateZ(0)] rounded-md bg-[#050505] p-1 [will-change:transform]">
    <div className="mb-[2px] flex w-full shrink-0 gap-[2px]">
      ...
      {[['F1',<IconBrightnessDown className="h-[6px] w-[6px]"/>],['F2',<IconBrightnessUp className="h-[6px] w-[6px]"/>],
        ['F3',<IconTable className="h-[6px] w-[6px]"/>],['F4',<IconSearch className="h-[6px] w-[6px]"/>],
        ['F5',<IconMicrophone className="h-[6px] w-[6px]"/>],['F6',<IconMoon className="h-[6px] w-[6px]"/>],
        ['F7',<IconPlayerTrackPrev className="h-[6px] w-[6px]"/>],['F8',<IconPlayerSkipForward className="h-[6px] w-[6px]"/>],
        ['F9',<IconPlayerTrackNext className="h-[6px] w-[6px]"/>],['F10',<IconVolume3 className="h-[6px] w-[6px]"/>],
        ['F11',<IconVolume2 className="h-[6px] w-[6px]"/>],['F12',<IconVolume className="h-[6px] w-[6px]"/>],
      ].map(([label, icon]) => (
        <KBtn key={label as string}>{icon}<span className="mt-1 inline-block">{label}</span></KBtn>
      ))}
    ...
    <div className="mb-[2px] flex w-full shrink-0 gap-[2px]">
      ...
      {'1234567890'.split('').map(n => <KBtn key={n}><span className="block">{n}</span></KBtn>)}
    ...
    <div className="mb-[2px] flex w-full shrink-0 gap-[2px]">
      ...
      {'QWERTYUIOP'.split('').map(l => <KBtn key={l}><span>{l}</span></KBtn>)}
      <KBtn><span>{'{'}</span><span>{'['}</span></KBtn><KBtn><span>{'}'}</span><span>{']'}</span></KBtn><KBtn><span>{'|'}</span><span>{'\\'}</span></KBtn>
    </div>
    <div className="mb-[2px] flex w-full shrink-0 gap-[2px]">
      <KBtn className="w-[2.8rem] items-end justify-start pb-[2px] pl-[4px]" childrenClassName="items-start">caps</KBtn>
      {'ASDFGHJKL'.split('').map(l => <KBtn key={l}><span>{l}</span></KBtn>)}
      <KBtn><span>:</span><span>;</span></KBtn><KBtn><span>"</span><span>'</span></KBtn>
    ...
    <div className="mb-[2px] flex w-full shrink-0 gap-[2px]">
      ...
      {'ZXCVBNM'.split('').map(l => <KBtn key={l}><span>{l}</span></KBtn>)}
      <KBtn><span>{'<'}</span><span>{','}</span></KBtn><KBtn><span>{'>'}</span><span>{'.'}</span></KBtn><KBtn><span>{'?'}</span><span>{'/'}</span></KBtn>
    ...
    <div className="mb-[2px] flex w-full shrink-0 gap-[2px]">
      ...
      <KBtn childrenClassName="h-full justify-between py-[4px]"><div className="flex w-full justify-end pr-1"><IconChevronUp className="h-[6px] w-[6px]"/></div><div className="flex w-full justify-start pl-1"><span>ctrl</span></div></KBtn>
      <KBtn childrenClassName="h-full justify-between py-[4px]"><div className="flex w-full justify-end pr-1"><OptionKey className="h-[6px] w-[6px]"/></div><div className="flex w-full justify-start pl-1"><span>opt</span></div></KBtn>
      <KBtn className="w-8" childrenClassName="h-full justify-between py-[4px]"><div className="flex w-full justify-end pr-1"><IconCommand className="h-[6px] w-[6px]"/></div><div className="flex w-full justify-start pl-1"><span>cmd</span></div></KBtn>
      <KBtn className="w-[8.2rem]"/>
      <KBtn className="w-8" childrenClassName="h-full justify-between py-[4px]"><div className="flex w-full justify-start pl-1"><IconCommand className="h-[6px] w-[6px]"/></div><div className="flex w-full justify-start pl-1"><span>cmd</span></div></KBtn>
      <KBtn childrenClassName="h-full justify-between py-[4px]"><div className="flex w-full justify-start pl-1"><OptionKey className="h-[6px] w-[6px]"/></div><div className="flex w-full justify-start pl-1"><span>opt</span></div></KBtn>
